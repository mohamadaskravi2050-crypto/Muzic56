    ...
    def post(self, request):
        try:
            ...
            if not title or not audio_file:
                return Response({'error': 'Title and audio file are required'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({
                ...
                'music': {
                    ...
                    'cover_url': request.build_absolute_uri(music.cover_image.url) if music.cover_image else None
                }
            })
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class MusicListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            serializer = MusicSerializer(music_list, many=True, context={'request': request})
...
class LikedMusicView(APIView):
    ...
    def get(self, request):
        try:
            ...
            serializer = MusicSerializer(liked_music, many=True, context={'request': request})
            return Response(serializer.data)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CreatePlaylistPageView(APIView):
    ...
    def get(self, request):
        try:
            ...
            serializer = MusicSerializer(all_music, many=True, context={'request': request})
            return Response(serializer.data)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            if playlist_id == 'liked_songs':
                ...
                playlist_data = {
                    ...
                    'songs': MusicSerializer(liked_songs, many=True, context={'request': request}).data
                }
            else:
                playlist = Playlist.objects.get(id=int(playlist_id), owner=request.user)
                playlist_songs = PlaylistSong.objects.filter(playlist=playlist).order_by('added_at')
                ...
                playlist_data = {
                    ...
                    'songs': MusicSerializer(songs, many=True, context={'request': request}).data
                }
            
            return Response(playlist_data)
            
        except Playlist.DoesNotExist:
            return Response({'error': 'Playlist not found or access denied'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def get(self, request):
        try:
            ...
            for playlist in playlists:
                ...
                # تعداد آهنگ‌ها - با روش ایمن
                try:
                    playlist_info['song_count'] = PlaylistSong.objects.filter(playlist=playlist).count()
                except Exception as e:
                    print(f"⚠️ Error counting songs for playlist {playlist.id}: {e}")
                    playlist_info['song_count'] = 0
                
                # عکس کاور - با روش ایمن
                try:
                    first_song_rel = PlaylistSong.objects.filter(playlist=playlist).first()
                    if first_song_rel and first_song_rel.song and first_song_rel.song.cover_image:
                        playlist_info['cover_url'] = request.build_absolute_uri(first_song_rel.song.cover_image.url)
                except Exception as e:
    ...
    try:
        ...
        try:
            ...
            for playlist_song in playlist_songs:
                ...
                # URL فایل‌ها با هندل کردن خطا
                try:
                    song_data['audio_url'] = request.build_absolute_uri(song.audio_file.url)
                except:
                    song_data['audio_url'] = None
                    
                try:
                    if song.cover_image:
                        song_data['cover_url'] = request.build_absolute_uri(song.cover_image.url)
                    else:
...
# در views.py این تابع را اضافه کنید:
...
def test_public_playlists(request):
    ...
    try:
        ...
        # تست ساده - فقط تعداد پلی‌لیست‌ها
...
class PopularMusicView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            serializer = MusicSerializer(popular_music, many=True, context={'request': request})
            return Response(serializer.data)
...
class SearchMusicView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            serializer = MusicSerializer(results, many=True, context={'request': request})
            return Response(serializer.data)
...
class DeleteMusicView(APIView):
    ...
    def delete(self, request, music_id):
        try:
            ...
            return Response({
                'success': False,
                'error': f'Error deleting music: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        

# در views.py این تابع را اضافه کن (در کنار سایر توابع)
@api_view(['GET'])
@authentication_classes([JWTAuthentication])
def public_playlist_detail(request, playlist_id):
    try:
        ...
        # چک کردن اینکه پلی‌لیست عمومی است
        playlist = Playlist.objects.get(id=playlist_id, is_public=True)
        
        # گرفتن آهنگ‌های پلی‌لیست
        playlist_songs = PlaylistSong.objects.filter(playlist=playlist).select_related('song')
        songs_data = []
        ...
        for playlist_song in playlist_songs:
            ...
            # اضافه کردن URL فایل‌ها با هندل کردن خطا
            try:
                if song.audio_file:
                    song_data['audio_url'] = request.build_absolute_uri(song.audio_file.url)
                else:
        ...
        print(f"✅ Successfully loaded playlist: {playlist.name} with {len(songs_data)} songs")
        return JsonResponse(playlist_data)
